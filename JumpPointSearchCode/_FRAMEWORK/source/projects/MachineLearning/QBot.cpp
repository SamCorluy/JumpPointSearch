...
void QBot::Reinforcement(float factor,int memory)
{
	...
	// Set deltabotbrains to zero
	m_DeltaBotBrain.SetAll(0);

	int mi{ min(m_MemorySize, memory) };

	for (int i = 0; i < mi; i++)
	{
		auto timeFactor = 1 / (1 + mi * mi);
		int actualIndex = currentIndex + i - mi;
		if (actualIndex < 0)
			actualIndex += m_MemorySize - 1;
		float max = m_ActionMatrixMemoryArr[actualIndex].Max();
		int cMax{};
		for (int c = 0; c < m_ActionMatrixMemoryArr->GetNrOfColumns(); ++c)
		{
			if (m_ActionMatrixMemoryArr->Get(actualIndex, c) == max)
			{
				cMax = c;
				break;
			}
		...
		{
			auto scVal = m_StateMatrixMemoryArr[currentIndex].Get(0, c);
			if(scVal > 0.0f) {
				m_DeltaBotBrain.Set(c, cMax, timeFactor * factor * scVal);
				auto rcMax = rand() % (m_DeltaBotBrain.GetNrOfColumns() - 1);
				if (rcMax == cMax)
					++rcMax;
				m_DeltaBotBrain.Set(c, rcMax, -timeFactor * factor * scVal);
			}
		}
	}
